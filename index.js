...
for (var test in out.results) {
  ...
  for (var n in run.subtests) {
    ...
    if (!run.subtests[n].totals.PASS) result.boom.push({ name: n, UAmessage: run.subtests[n].UAmessage, byUA: run.subtests[n].byUA });
    for (var i = 0, m = out.ua.length; i < m; i++) {
      var res = run.subtests[n].byUA[out.ua[i]];
      if (res === "PASS") uaPass[out.ua[i]]++;
    }
    result.subtests.push({ name: n, UAmessage: run.subtests[n].UAmessage, byUA: run.subtests[n].byUA });
  }
...
var script = options.failures ? "window.setTimeout(function() { \n $('.message_toggle').show();\n$('.message_toggle').on('click', function() {\n$('.messages').toggle();\n});\n}, 1000);" : "";
